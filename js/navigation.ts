...
import { getFilteredGames, setFilterLetter, setFilterFav, AL, updateUI, filterLetter, filterFav } from './ui.js';
...
export function navigate(dir) {
  ...
  // ── Pas de filtre actif : navigation dans TOUS les jeux sans limite ──
  if (!filterLetter && !filterFav) {
    const nextIdx = (currentIdx + dir + allGames.length) % allGames.length;
  ...
  // ── Filtre actif : navigation dans les jeux filtrés ──
  const filtered = getFilteredGames();
  if (filtered.length === 0) return;

  const currentGame = allGames[currentIdx];
  const currentFilteredIdx = filtered.indexOf(currentGame);
  let nextFilteredIdx = (currentFilteredIdx !== -1 ? currentFilteredIdx : 0) + dir;

  if (nextFilteredIdx < 0 || nextFilteredIdx >= filtered.length) {
    // Limite atteinte → cherche la lettre adjacente
    const currentLetter = filterLetter || '#';
    const currentLetterIdx = AL.indexOf(currentLetter);
    ...
    for (let i = 1; i < AL.length; i++) {
      ...
      if (hasGames) {
        ...
        if (newFiltered.length > 0) {
          ...
          setCi(newGameIdx);
          const prevGame = allGames[(newGameIdx - 1 + allGames.length) % allGames.length];
          const nextGame = allGames[(newGameIdx + 1) % allGames.length];
          updateGameDisplay(targetGame, prevGame, nextGame, newFiltered, allGames);
          schedulePreload(dir > 0 ? 0 : newFiltered.length - 1, newFiltered);
        }
        return;
      }
    }
    // Pas d'autre lettre → wrap dans la lettre courante
    nextFilteredIdx = (nextFilteredIdx + filtered.length) % filtered.length;
  }

  const game = filtered[nextFilteredIdx];
  if (!game) return;
  const gameIdx = allGames.indexOf(game);
  const prevGame = allGames[(gameIdx - 1 + allGames.length) % allGames.length];
...
export function navigateToLetter(dir) {
  ...
  if (foundLetter) {
    ...
    if (filtered.length > 0) {
      ...
      setCi(newIdx);
      // allGames pour les vrais voisins cross-lettre
      const prevGame = allGames[(newIdx - 1 + allGames.length) % allGames.length];
      const nextGame = allGames[(newIdx + 1) % allGames.length];
      updateGameDisplay(game, prevGame, nextGame, filtered, allGames);
    }
  }
}
