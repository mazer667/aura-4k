...
export function getSetting(key)    { return settings[key] ?? DEFAULTS[key]; }
...
export function getTransitionDuration() {
  const map = { fast: 250, normal: 500, slow: 1000, none: 0 };
...
function activateTab(index) {
  ...
  document.querySelectorAll('.aura-opt-nav-item').forEach(el =>
    el.classList.toggle('active', el.dataset.tab === tabName)
  );
  document.querySelectorAll('.aura-opt-tab').forEach(el =>
...
async function selectRomsFolder() {
  ...
  document.getElementById('aura-selected-folder').textContent = selectedRomsFolder;
  
  const countResult = await window.electronAPI.countRoms(selectedRomsFolder);
  const gamesLabel = t('data.games');
  document.getElementById('aura-roms-count').innerHTML = countResult.count + ' <span data-i18n="data.games">' + gamesLabel + '</span>';
}
...
async function createXml() {
  ...
  document.getElementById('aura-data-result').style.display = 'none';
  document.getElementById('aura-scan-progress').style.display = 'block';
  
  const result = await window.electronAPI.createXml(consoleName, selectedRomsFolder);
  
  document.getElementById('aura-scan-progress').style.display = 'none';
...
function bindEvents() {
  ...
  document.getElementById('aura-opt-overlay')
    ?.addEventListener('click', e => { if (e.target.id === 'aura-opt-overlay') closeOptions(); });
  ...
  document.querySelectorAll('[data-group="speed"]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      saveSetting('transition_speed', btn.dataset.value);
  ...
  document.querySelectorAll('.aura-accent-dot').forEach(dot => {
    dot.addEventListener('click', () => {
      ...
      saveSetting('accent_color', dot.dataset.color);
      applyAccent();
    });
  });
  ...
  document.querySelectorAll('.aura-lang-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      saveSetting('language', card.dataset.lang);
      applyLanguage(card.dataset.lang); // ← traduit tout le DOM immédiatement
  ...
  let listeningKey  = null;  // action en cours d'assignation
  let gpPollId      = null;  // requestAnimationFrame ID
  ...
  function buildGpActions() {
    ...
    GP_ACTIONS.forEach(action => {
      const btnIdx  = settings[action.key] ?? DEFAULTS[action.key];
  ...
  document.addEventListener('click', e => {
    if (listeningKey && !e.target.closest('.aura-gp-action')) stopListening(false);
  });
...
function bindStepper(valId, decId, incId, options, callback) {
  ...
  document.getElementById(decId)?.addEventListener('click', () => {
    ...
    if (idx > 0) {
      valEl.textContent = options[idx - 1];
      callback(options[idx - 1]);
    }
  });
