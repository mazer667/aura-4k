...
import { getFilteredGames, setFilterLetter, AL, updateUI, filterLetter, filterFav } from "./ui.js";
...
function navigate(dir) {
  ...
  if (!filterLetter && !filterFav) {
    ...
    const game2 = allGames[nextIdx];
    if (!game2) return;
    const prevGame2 = allGames[(nextIdx - 1 + allGames.length) % allGames.length];
    const nextGame2 = allGames[(nextIdx + 1) % allGames.length];
    updateGameDisplay(game2, prevGame2, nextGame2, allGames, allGames);
    ...
    for (let i = 1; i < AL.length; i++) {
      ...
      if (hasGames) {
        ...
        if (newFiltered.length > 0) {
          ...
          setCi(newGameIdx);
          const prevGame2 = allGames[(newGameIdx - 1 + allGames.length) % allGames.length];
          const nextGame2 = allGames[(newGameIdx + 1) % allGames.length];
          updateGameDisplay(targetGame, prevGame2, nextGame2, newFiltered, allGames);
