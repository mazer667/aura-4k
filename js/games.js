...
function getConsoleConfigByName(consoleName) {
...
function isGameImageCached(game) {
  if (!game) return false;
  return isImageCached(imgPath(game));
}
function getRomExtension(consoleName) {
...
function getRomExtensions(game) {
  const consoleConfig = getConsoleConfigByName(game.console);
  if (!consoleConfig?.romExtensions?.length) return [".zip"];
...
function getMusicId(game) {
  if (!game || !game.rom) return null;
  return game.rom;
}
function getMusicUrl(game) {
...
async function loadGamesAndPopulate(consoleName = "FBNeo - Arcade Games", forceReload = false) {
